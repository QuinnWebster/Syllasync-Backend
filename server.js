...
const sendEvents = require("./email.js");
...
app.post("/send-events", async (req, res) => {
  try {
    if (!req.body.events || !req.body.recipientEmail) {
      res
        .status(400)
        .json({ error: "Please provide events and recipient email." });
    }
    sendEvents(req.body.events, req.body.recipientEmail);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
